...
class CustomApi
{
    ...
    public function connect($params)
    {
        ...
        $comm100WebhookToken = $params['webhookToken'];
        ...
        try {
            ...
            $updateData = [
                ...
                "Comm100AccessToken" => $comm100WebhookToken,
    ...
    public function getStores()
    {
        ...
        $customApiResponse->setApiResponse(true, $returnArray);
        header('Content-Type: application/json; charset=utf-8');
        $returnArray = json_encode($customApiResponse->getApiResponse());
        print_r($returnArray, false);
    }
    ...
    public function getWebsiteStores()
    {
        ...
        foreach ($websites as $website) {
            ...
            $website['stores'] = [];
            $websiteStores = $storeCollection
                ->addWebsiteFilter($website['website_id'])
                ->getData();
            foreach ($websiteStores as $websiteStore) {
                array_push($website['stores'], $websiteStore);
            }
            array_push($responseWebsites, $website);
        }

        // foreach ($storeManagerDataList as $key => $value) {
        //     $returnArray[] = ['storeId' => $value['store_id'], 'code' =>  $value['code'], 'name' => $value['name']];
        // }

        $customApiResponse->setApiResponse(true, $responseWebsites);
        header('Content-Type: application/json; charset=utf-8');
        $finalWebsites = json_encode($customApiResponse->getApiResponse());
        print_r($finalWebsites, false);
    }

    /**
     * POST Set stores and campaigns.
     *
     *
     */
    ...
    {
        $webhookConnectionStatus = $this->getComm100MagentoDbDetails()['WebhookConnectionStatus'];
        if ($webhookConnectionStatus == null) {
            $webhookConnectionStatus = false;
        } else {
            $webhookConnectionStatus = $webhookConnectionStatus == 1;
        }

        $customApiResponse = new CustomApiResponse();
        $customApiResponse->setApiResponse(true, [
    ...
    {
        // $comm100LiveChatData = ['consumerKey' => '', 'consumerSecret' => '', 'oAuthVerifier' => ''];
